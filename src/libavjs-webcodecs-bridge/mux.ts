...
export async function configToAudioStream(
    libav: LibAVJS.LibAV, config: AudioEncoderConfig
...
export async function configToVideoStream(
    libav: LibAVJS.LibAV, config: VideoEncoderConfig | VideoEncoderConfig
): Promise<[number, number, number]> {
    const codecLong = config.codec;
    let codec: string;
    // if (typeof codecLong === "object")
 ...
 */
function times(chunk: EncodedAudioChunk | EncodedVideoChunk | EncodedVideoChunk, stream: [number, number, number]) {
    const num = stream[1];
    const den = stream[2];
    return {
...
function encodedChunkToPacket(
    chunk: EncodedAudioChunk | EncodedVideoChunk | EncodedVideoChunk,
...
export async function encodedAudioChunkToPacket(
    libav: LibAVJS.LibAV, chunk: EncodedAudioChunk, metadata: any,
    stream: [number, number, number], streamIndex: number
...
export async function encodedVideoChunkToPacket(
    libav: LibAVJS.LibAV, chunk: EncodedVideoChunk | EncodedVideoChunk, metadata: any,
    stream: [number, number, number], streamIndex: number
): Promise<LibAVJS.Packet> {
